...
st.markdown('<div class="title">Radar Crisi d’Impresa — Turnaround 4.2</div>',unsafe_allow_html=True)
st.markdown('<div class="sub">Evidence-first extraction · Normalization Gate · Business Plan 5Y</div>',unsafe_allow_html=True)
st.info('Principio 4.2: il business plan è BLOCCATO finché i dati contabili non sono coerenti, le rettifiche materiali non sono chiuse e il professionista non conferma la base normalizzata.')
...
with T1:
    ...
    if 'fields42' in st.session_state:
        ...
        st.caption('La 4.2 non propone rettifiche sulla sola presenza di parole generiche. Serve evidenza di eccezionalità/non ricorrenza e un importo collegabile. Solo VERIFICATA entra nei calcoli; ESCLUSA chiude la proposta senza effetto.')
...
with T2:
    if 'validated42' not in st.session_state or 'nm42' not in st.session_state:
        ...
        filename=f"Radar_Turnaround_{(d.get('ragione_sociale') or 'Target').replace(' ','_')}_4.2.xlsx"
        st.download_button('ESPORTA REPORT TURNAROUND 4.2',report,filename,'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',use_container_width=True)
...
with T3:
    if not st.session_state.portfolio42: st.info('Nessun target nella sessione corrente.')
        ...
        out=io.BytesIO(); df.to_excel(out,index=False); out.seek(0); st.download_button('ESPORTA PORTAFOGLIO',out,'Portafoglio_Radar_4.2.xlsx','application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')

with T4:
    st.markdown('''**Architettura 4.2**
