...
from reporting import decision_level, build_ratios, build_thesis, document_checklist, make_excel
...
st.info('Privacy by design: il PDF viene elaborato in memoria durante la sessione e non viene salvato nel portafoglio. Il portafoglio della 3.0 è di sessione e si perde al riavvio dell’app, salvo esportazione manuale.')
    ...
    if up:
        ...
        if 'fields_raw' in st.session_state:
            ...
            if not core_ok:
                ...
                sc=score(edited,qual); decision=decision_level(sc,quality,qual['red_flag']); ratios=build_ratios(edited); thesis=build_thesis(edited,sc); checklist=document_checklist(edited,sc)
                ...
                m1,m2,m3,m4=st.columns(4); m1.metric('Punteggio Radar',f"{sc['totale']}/100"); m2.metric('Classe',sc['classe']); m3.metric('Tax Turnaround Fit',sc['fit']); m4.metric('Distress composito',f"{sc['composito']}/40")
                a,b=st.columns(2)
                with a:
                ...
                c2.download_button('ESPORTA SCHEDA EXCEL',excel,file_name=f"Radar_{(edited.get('ragione_sociale') or 'Target').replace(' ','_')}_3.0.xlsx",mime='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',use_container_width=True)
...
with T2:
    ...
    if not p: st.info('Nessun target salvato nella sessione corrente.')
        ...
        out.seek(0); st.download_button('ESPORTA PORTAFOGLIO',out,'Portafoglio_Radar_3.0.xlsx','application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
        if st.button('SVUOTA PORTAFOGLIO DI SESSIONE'):
            st.session_state['portfolio']=[]; st.rerun()

with T3:
    st.markdown('''**Controlli attivi nella versione 3.0**
